...
void bellman_ford(ull no_nodes, vector<Edge> edges, vector<string> currencies) {
  auto dist = vector<double>(no_nodes, 0);
  ...
  rep(it, no_nodes + 1) {
    ...
    for (const auto& edge : edges) {
      ...
      const auto er = edge.cost;

      if (dist[target] < dist[start] * er) {
        cout << "(" << it << ") Exchanging: " << currencies[start] << " to " << currencies[target] << endl;
        cout << "Prev: " << dist[target] << ", then: " << dist[start] + length << endl;
