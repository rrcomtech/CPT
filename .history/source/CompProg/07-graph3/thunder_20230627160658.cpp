...
bool isConnected(Graph& graph) {
  ...
  return visited_elements == graph.size();
}
...
pair<ll, double> modified_dijkstra(Graph& graph, int start) {
  ...
  while (!q.empty()) {
      ...
      for (const auto& [a, b] : graph[current_index]) {
...
void solve() {
  ...
  if (!isConnected(graph)) {
    ...
    auto max_distance = double{0};
    rep(i, actual_cities) {
      const auto [_, new_res] = modified_dijkstra(graph, i);
      max_distance = max(max_distance, new_res);
    }
