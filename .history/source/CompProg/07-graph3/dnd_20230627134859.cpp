...
void solve(Graph graph, ull start, ull end, ull lengthLongestEdge) {
  auto buckets = unordered_map<ull, vector<ull>>();
  unordered_map<ull, vector<ull>> buckets;
...
int main() {
  ...
  rep(i, edges) {
    ...
    longestEdge = max(longestEdge, cost);
  }
