...
using uint = uint32_t;
‚
void solve(Graph graph, ull start, ull end, vector<bool>& processed, vector<unordered_map<ull,ull>>& cache) {
  if (processed.at(start)) {
    if (cache[start][end] == SIZE_MAX) {
      cout << "BRIBE DM WITH FOOD (cached)" << endl;
    } else {
...
int main() {
  ...
  cache = vector<unordered_map<ull,ull>>(nodes + 1);
  processed = vector<bool>(nodes + 1, false);
